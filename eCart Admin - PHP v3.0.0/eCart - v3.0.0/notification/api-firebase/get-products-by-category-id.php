...
if (isset($_POST['accesskey']) && isset($_POST['category_id'])) {
    ...
    if ($access_key_received == $access_key) {
        ...
        $sql = "SELECT p.*,(SELECT name FROM category c WHERE c.id=p.category_id) as category_name,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM products p WHERE `status`=1 and subcategory_id=0 and category_id='" . $subcategory_id . "' $sort LIMIT $offset, $limit";
        $db->sql($sql);
        $res = $db->getResult();
        $product = array();
