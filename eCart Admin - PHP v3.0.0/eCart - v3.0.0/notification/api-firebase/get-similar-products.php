...
if (isset($_POST['accesskey']) && isset($_POST['get_similar_products']) && !empty($_POST['get_similar_products'])) {
    ...
    if ($access_key_received == $access_key) {
        ...
        $sql = "SELECT p.*,(SELECT c.name FROM category c WHERE c.id=p.category_id) as category_name,(SELECT MIN(pv.price) FROM product_variant pv WHERE pv.product_id=p.id) as price FROM products p where id != $product_id and `status`=1 and category_id = $category_id ORDER BY $order LIMIT $offset,$limit";
        $db->sql($sql);
        $res = $db->getResult();
        if (!empty($res)) {
            foreach ($res as $row) {
                ...
                $tempRow['shipping_delivery'] = $row['shipping_delivery'];
                ...
                $tempRow['size_chart'] = (empty($row['size_chart'])) ? '' : DOMAIN_URL . $row['size_chart'];
