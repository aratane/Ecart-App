...
class custom_functions
{
    ...
    public function validate_other_images1($tmp_name, $type)
    {
        if (function_exists('finfo_file')) {
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $type = finfo_file($finfo, $tmp_name);
    ...
    public function get_products($user_id = NULL, $id = NULL, $slug = NULL, $category_id = NULL, $subcategory_id = NULL, $where = '', $limit = 10, $offset = 0, $sort = "p.id", $order = "DESC", $group_by_product = "", $group_by_variant = "", $context = 1, $search = '', $field = "", $join_table = "", $where1 = "")
    {
        if ($sort == 'new') {
            $sort = ' date_added ';
            $order = 'DESC';
        ...
        // echo $sql;
        $sql = "SELECT p.*,c.name as category_name, s.name as subcategory_name, t.title as tax_title, t.percentage as tax_percentage,
        ...
        if (!empty($res)) {
            foreach ($res as $row) {
                // print_r($row);
                // $row['is_added_to_cart'] = (isset($row['is_added_to_cart']) && $row['is_added_to_cart'] != NULL) ? true : false;
                // $row['is_favorite'] = (isset($row['is_favorite']) && $row['is_favorite'] != NULL && $row['is_favorite'] > 0) ? true : false;
                ...
                $sql = "SELECT *,(SELECT short_code FROM unit u WHERE u.id=pv.measurement_unit_id) as measurement_unit_name,(SELECT short_code FROM unit u WHERE u.id=pv.stock_unit_id) as stock_unit_name FROM product_variant pv WHERE pv.product_id=" . $row['id'] . " $where1 $group_by_variant $price_sort";
                $this->db->sql($sql);
                ...
                for ($k = 0; $k < count($variants); $k++) {
                    ...
                    $sql = "SELECT fp.*,fs.title as flash_sales_name FROM flash_sales_products fp LEFT JOIN flash_sales fs ON fs.id=fp.flash_sales_id where fp.product_variant_id= " . $variants[$k]['id'] . " AND  fp.product_id = " . $variants[$k]['product_id'] . " GROUP BY fp.id";
                    $this->db->sql($sql);
