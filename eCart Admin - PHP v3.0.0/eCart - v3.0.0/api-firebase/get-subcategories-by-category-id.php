...
if (isset($_POST['accesskey']) && isset($_POST['category_id'])) {
    ...
    if ($access_key_received == $access_key) {
        $sql_query = "SELECT *,(SELECT name FROM category c WHERE subcategory.category_id=c.id) as category_name FROM subcategory WhERE category_id='" . $category_id . "' ORDER BY id DESC LIMIT " . $offset . ", " . $limit;
        $db->sql($sql_query);
        $res = $db->getResult();
        $db->disconnect();
        if (!empty($res)) {
